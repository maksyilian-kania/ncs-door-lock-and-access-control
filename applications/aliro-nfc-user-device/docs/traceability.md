...
| ALIRO-UD-SYRS-P1-008 | Never export Access Credential private key, Kpersistent, or session key via CLI/NFC/logs. | `src/platform/crypto/{crypto,certificate,credential_signing}.cpp` (AWP5: every PSA key is referenced only by opaque `KeyId`; `CredentialSigning::Sign()` resolves a `CredentialHandle` to its PSA key and signs through `KeyBackend::Sign()` without ever copying the scalar into this module), `src/storage/credential` (opaque PSA key IDs only, never a raw scalar) | Kpersistent generation/storage (AWP5/AWP6 orchestration) | AWP1-5 "Security check" grep review of every changed file each AWP (see `docs/evidence.md`); `aliro_ud_credential_signing` suite | — | ALIRO-SPEC §16.2; NORDIC-SEC | app-implemented (credential-storage and session/ephemeral-key portions; Kpersistent/mailbox portions remain AWP6) |
