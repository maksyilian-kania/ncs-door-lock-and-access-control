...
/*
 * The generated header contains the model data and defines model_fomo. Include
 * it in exactly this translation unit.
 */
...
{
	/*
	 * The Axon NPU emits multi-channel tensors in CHW (channel-major)
	 * layout, not the HWC (channel-interleaved) layout TFLite/Edge
	 * Impulse tooling assumes: all kOutputWidth*kOutputHeight values for
	 * channel 0 (background) come first, then all values for channel 1
	 * ("hand"). Indexing this as [row][column][channel] silently reads
	 * the wrong bytes for every cell -- see the CHW/HWC mismatch
	 * documented for Edge Impulse's Axon inference backend
	 * (nordic_axon.h) for the same root cause in a related project.
	 * Its scale is 1/256 and zero point is -128, so q + 128 is the
	 * unsigned probability numerator.
	 */
...
int Run(const uint8_t *frame, size_t frameSize, Result &result)
{
	...
	/*
	 * The camera provides unsigned grayscale and the model input has scale
	 * 1/255 with zero point -128. Therefore the exact quantized input is
	 * gray - 128; no float feature matrix or image DSP step is needed.
	 */
