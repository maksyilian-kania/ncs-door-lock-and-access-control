...
/*
 * uart_fifo_fill() and the other interrupt-driven UART calls must run in the
 * context of the work queue that drives the CDC ACM class, and the driver
 * asserts when they do not. The default is the system work queue, which
 * accepts any context and is what lets FrameTxHandler() push a frame from
 * gesture_access_workqueue - see frame_stream.c in sdk-edge-ai's
 * photo_gesture_detection sample for the same constraint.
 */
...
/*
 * Scratch chunk size used both for metadata and pixel bytes on the way out.
 * Pixels here are already raw grayscale bytes (unlike the RGB565 sample,
 * which decodes into this buffer), so this only needs to be big enough to
 * keep the USB pipe fed, not to hold anything frame-sized - see the RAM
 * discussion in gesture_access.cpp around sFrameBuf.
 */
...
/* Bus powered, no remote wakeup, 250 mA. */
